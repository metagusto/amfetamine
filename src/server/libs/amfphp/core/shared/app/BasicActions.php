...
function classLoaderAction(&$amfbody)
{
    ...
    if (!$amfbody->noExec) {
        ...
        if (!$fileIncluded) {
            ...
            Logger::getInstance()->error("The class file {" . $amfbody->className . "} exists but could not be included. The file may have syntax errors, or includes at the top of the file cannot be resolved.");
            MessageException::throwException($amfbody, $ex);
            return false;
        }
        ...
        if (!class_exists($amfbody->className)) {
            ...
            Logger::getInstance()->error("The file {" . $amfbody->className . ".php} exists and was included correctly but a class by that name could not be found in that file. Perhaps the class is misnamed.");
            MessageException::throwException($amfbody, $ex);
            return false;
        }
