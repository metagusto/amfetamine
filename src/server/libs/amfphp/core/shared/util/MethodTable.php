...
class MethodTable
{
    ...
    public static function getClassMethodsReflection($sourcePath, $className, & $classComment)
    {
        ...
        foreach ($methods as $reflectionMethod) {
            // TODO: Perhaps authors means $reflectionMethod instead $method? :|
            if ($reflectionMethod->isPublic() && $reflectionMethod->name [0] != '_' && $reflectionMethod->name != 'beforeFilter') //f ($reflectionMethod->isPublic() && $method->name [0] != '_' && $method->name != 'beforeFilter')
            {
                $parameters = array();
    ...
    public static function getMethodArguments($methodSignature, $commentParams)
    {
        if (strlen($methodSignature) < 2) {
            ...
            /**
             * metagûsto fix. Structure doesn't fit documentation
             */
            //clean the arguments before returning them
            $temp = explode(",", $methodSignature);
    ...
    /**
     * New implementation of cleanArguments
     *
     * @author metagûsto
     * @param array $args
    ...
    public static function cleanArguments($args, $commentParams)
    {
        ...
        foreach ($args as $index => $arg) {
            ...
            if (!isset($commentParams [$index]))
                ...
                if ($end != "" && $paramName != "" && strpos(strtolower($end), strtolower($paramName)) === 0)
                    $end = substr($end, strlen($paramName));

                // Getting parameter Type
                $paramType = self::getParameterType($end);

                // Getting parameter description
                $paramDesc = self::getParameterDescription($end);

                // Checking if requested
                $paramRequired = self::isParameterRequired($paramDesc);

                // Building parameter metadata array
                $metaArray = array();
                $metaArray ["type"] = $paramType;
                $metaArray ["required"] = $paramRequired;
                $metaArray ["description"] = $paramDesc;
                $result [$paramName] = $metaArray;

                /**
                 * Note.
                 * This double metainformation seems the only way to fix both ServiceBrowser (binary) and Amf Type Matching :(
                 * At the present, we're not able to get the src for Service Browser... wasn't it Open Source !? :|
                 */
                $result [$counter] = $paramName;
            }
            $counter++;
        }
        return $result;
     ...
     * @return unknown
     */
    public static function getParameterDescription($source)
    {
        $partCount = 0;
    ...
    /**
     * Parse parameter description detecting if the parameter is required or not
     *
     * @param string $desc . The method description
     * @return boolean
     */
    public static function isParameterRequired($desc)
    {
     ...
     */
    protected static function __cleanArguments($args, $commentParams)
    {
        $result = array();
     ...
     *
     */
    public static function showCode($methodTable)
    {
