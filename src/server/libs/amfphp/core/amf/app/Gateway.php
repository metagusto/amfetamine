...
class Gateway
{
    public $_looseMode = false;
    public $_charsetMethod = "none";
    public $_charsetPhp = "";
    public $_charsetSql = "";
    public $exec;
    public $filters;
    public $actions;
    public $outgoingMessagesFolder = NULL;
    public $incomingMessagesFolder = NULL;
    public $useSslFirstMethod = true;
    public $_enableGzipCompression = false;
    ...
    function service()
    {
        ...
        if (isset($GLOBALS ["HTTP_RAW_POST_DATA"]) && $GLOBALS ["HTTP_RAW_POST_DATA"] != "") {
            ...
            print $this->generateWelcomeMessage();
        }
    }
    ...
    /**
     * Generates AMFetamine welcome message
     * @return string
     */
    protected function generateWelcomeMessage()
    {
        $buffer = "";
        $buffer .= "<div align='center'>";
        if ($GLOBALS ['amfphp'] ['disableDebug'] == false) {
            $buffer .= "<hr /><p style='font-family: consolas, courier new; font-size: 14px'>Your Gateway seems correctly installed.</p>";
            $buffer .= "<hr />";

            if (function_exists("amf_decode"))
                $buffer .= "<p>AMF C Extension is loaded " . ($GLOBALS ['amfphp'] ['native'] ? "and enabled." : "but disabled") . "</p>";

            if ($GLOBALS ['amfphp'] ['disableDebug'] == false)
                $version = "release: " . AMFPHP_VERSION;
            else
                $version = "";
            $buffer .= "<p style='font-family: consolas, courier new; font-size: 12px'><b>Metag&ucirc;sto AMFetamine Server</b> $version</p>";
        }
        else {
            $buffer .= "<hr /><p style='font-family: consolas, courier new; font-size: 12px'><b>Metag&ucirc;sto AMFetamine Server</b></p><hr />";
        }
        $buffer .= "</div>";
        return ($buffer);
    }
