...
class AMFBaseDeserializer
{
    ...
    function decodeCallback($event, $arg)
    {
        if ($event == 1) //Object
        {
            ...
            if ($event == 2) //Object post decode
            {
                ...
                if ($event == 3) //XML post-decode
                {
                    return $arg;
                }
                else
                    if ($event == 4) //Serializable post-decode
                    {
                        if ($type == 'flex.messaging.io.ArrayCollection' || $type == 'flex.messaging.io.ObjectProxy') {
                            ...
                            trigger_error(
                                "Unable to read externalizable data type " . $type,
                                E_USER_ERROR);
                            return "error";
                        }
                    }
                    else
                        if ($event == 5) //ByteArray post decode
                        {
    ...
    function readInt()
    {
        return ((ord($this->raw_data[$this->current_byte++]) << 8) | ord($this->raw_data[$this->current_byte++])); // read the next 2 bytes, shift and add
    }
    ...
    function readLong()
    {
        return ((ord($this->raw_data[$this->current_byte++]) << 24) | (ord($this->raw_data[$this->current_byte++]) << 16) | (ord(
                            $this->raw_data[$this->current_byte++]) << 8) | ord($this->raw_data[$this->current_byte++])); // read the next 4 bytes, shift and add
    }
