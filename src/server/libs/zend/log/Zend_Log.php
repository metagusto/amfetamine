...
#//require_once 'filter/Zend_Log_Filter_Priority.php';
...
class Zend_Log
{
    ...
    public function __construct(Zend_Log_Writer_Abstract $writer = null)
    {
        ...
        foreach ($this->_priorities as $index => $label) {
            $this->_priorities[$index] = substr($this->_priorities[$index] . "      ", 0, 6);
        }
    }
    ...
    public function log($message, $priority)
    {
        ...
        //$event = array_merge(array('timestamp'    => date('d/m/Y H:i:s'),
        $now = new Date();
        $event = array_merge(array('timestamp' => $now->format("d/m/Y H:i:s"),
                'message' => $message,
                'priority' => $priority,
    ...
    /**
     * Returns Zend_Log_Writer_Abstract
     *
     * @param integer $index
